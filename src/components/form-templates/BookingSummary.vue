...
export default {
  ...
  computed: {
    ...
    bookingSummary: function () {
      ...
      // Total Received
      const totalReceived = roundToTwoDecimal(this.clientMeta.amountReceived);
  ...
  watch: {
    queryParams: {
      ...
      handler: async function (v) {
        const data = {
          ...
          ...(v.addons && {
            addons: v.addons
              ...
              .filter(
                ({ paymentStatus }) =>
                  paymentStatus !== "REFUNDED" && paymentStatus !== "VOIDED",
              )
        ...
        this.$emit("totalPayment", this.room[0].roomTotal);
      },
    },
  },
};
