<template>
  <div>
    <div v-if="loading">
      <v-skeleton-loader type="heading" width="50%" height="32" />
        ...
        <v-col cols="12" md="6">
          ...
          <FormSection title="Guest Name">
            <v-autocomplete
              ...
              :disabled="getLoading('transactions')"
...
export default {
	...
	methods: {
		...
		async handleClick(referenceNum, fullName) {
			...
			this.tempAddons = this.transaction?.priceSummary?.fullAddons?.map(
				(fa) => ({
					...fa,
					name: fa.name.trim(),
