...
export default {
  ...
  data: () => ({
    routes: {
      ...
      CHECKED_IN: "Guest CheckInOut",
      CHECKED_OUT: "Guest CheckInOut",
    },
  }),
  methods: {
    ...
    handleCreateTransaction: function (payload) {
      const formattedPayload = this.createObject(payload);

      this.createTransaction(formattedPayload).then((response) => {
        // Clear Temp Data and assign route path
        this.clearTempData();
        const route = this.routes[response.data.results.status];
