...
export const reports = {
	...
	getters: {
		...
		getCashierReport: (state) => (id) =>
			state.cashierReports.find(
				({ cashierId }) => String(cashierId) === String(id),
			),
	...
	mutations: {
		SET_GUEST_REPORTS: (state, data) => {
			state.guestReports = data;
		},
		...
		SET_CASHIER_REPORTS: (state, data) => {
			state.cashierReports = data;
		},
	...
	actions: {
		...
		async fetchCashierReports({ commit }, queryParams) {
			...
			try {
				...
				const cashierReports = data.data;
